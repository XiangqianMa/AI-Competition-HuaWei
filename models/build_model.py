...
from models.custom_fine_grained_model import CustomFineGrainedModel
...
class PrepareFineGrainedModel():
    ...
    def create_model(self, model_type, classes_num, fine_grained_num_classes, last_stride, droprate, pretrained=True):
        """创建模型
        Args:
            model_type: 模型类型
            last_stride: resnet最后一个下采样层的步长；类型为int
        ...
        """
        print('Creating model: {}'.format(model_type))
        model = CustomFineGrainedModel(model_type, classes_num, fine_grained_num_classes, last_stride, droprate, pretrained=pretrained)
        return model
